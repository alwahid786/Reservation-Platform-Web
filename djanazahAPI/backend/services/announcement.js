...
const { loadTemplate } = require("../ulti/loadfile");
...
exports.AnnouncementServices = {
  ...
  CreateAnnouncement: async (udid, formValue) => {
    ...
    janazahDetails.isFixedTime = formValue.janazahDetails.isFixedTime ? formValue.janazahDetails.isFixedTime.value : false;
    janazahDetails.fixedTime = formValue.janazahDetails.fixedTime ? formValue.janazahDetails.fixedTime.value : "";
    if (!janazahDetails.isFixedTime) {
      if (formValue.janazahDetails.time.value !== null)
  ...
  UpdateAnnouncement: async (id, formValue) => {
    ...
    updatedAnnouncement.isPushNoti = false;
  ...
  //     if (type === "update") {
  //       title = "Djanazah Gebeden: Update Announcement!";
  ...
  UpdateDeviceAnnouncement: async (formValue) => {
    ...
    newAnnouncement.isPushNoti = false;
  ...
  DeleteDeviceAnnouncement: async (id) => {
    ...
    if (deletedAnnouncement) {
      await CommentModel.deleteMany({
        announcementId: deletedAnnouncement._id,
      });
  ...
  SendEmail: async () => {
    let warning = "";
    const email = await EmailModel.findOne({ isDefault: true });
    if (email) {
      try {
        const mainEmail = email.email;
        const mainPassword = email.password;
        ...
        let htmlstream = await loadTemplate();
        htmlstream = htmlstream.replace("${1}", "Hello");
        htmlstream = htmlstream.replace("${2}", "Djanazah Gebeden");

        // send mail with defined transport object
        await transporter.sendMail({
          from: `"Fred Foo 👻" <${mainEmail}>`,
          to: mainEmail,
    ...
    } else warning = "Default email is not set";
  ...
  SendNotificationById: async (id, type, deletedAnnouncement = null) => {
    ...
    if (announcement.udid !== "deleted") {
      ...
      if (registrationTokens.length > 0) {
        //#region Create Announcement
        let title = `Djanazah Gebed: New Announcement!`;
        let body = `${ConvertToDate(announcement.janazahDetails.date)} | ${
          announcement.janazahDetails.city
        } | ${announcement.janazahDetails.name}`;
        //#endregion
        ...
        //#endregion
        var newArrays = [];
        while (registrationTokens.length) {
          newArrays.push(registrationTokens.splice(0, 400));
        }
        for (let i in newArrays) {
          let registrationTokenArray = newArrays[i];
          let message = {
            notification: {
              title: title,
              body: body,
